...
export const login = async (req, res) => {
  try {
    ...
    if (!logeo) {
      return res
        .status(401)
        .json({ cod: 401, msj: "Email o contrasena incorrectos", datos: null });
    }
    ...
    return res.json({
      cod: 200,
      msj: "Login exitoso",
      datos: { usuario: logeo, token: token },
    });
  } catch (error) {
...
export async function register(req, res) {
  try {
    ...
    if (existente) {
      return res
        .status(409)
        .json({ cod: 409, msj: "El email ya está registrado", datos: null });
    }

    const result = await modeloAuth.register(
      us_nombre,
      us_apellido,
      us_email,
      us_contrasena,
    ...
    res
      .status(500)
      .json({ cod: 500, msj: "Error en el servidor", datos: null });
  }
}
